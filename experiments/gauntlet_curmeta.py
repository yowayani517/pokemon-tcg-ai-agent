"""プロセス隔離ガントレット: 1試合ごとにrun_one_arch.pyをsubprocessで起動。"""
...
def run_match(oppspec, n):
    ...
    for g in range(n):
        ...
        out = subprocess.run([PY, os.path.join(HERE, "run_one_arch.py"), oppspec, str(seat)],
...
def main():
    print(f"=== Archaludon vs 現メタ (isolated, {N} games each) ===", flush=True)
    w, l, d = run_match("wall", N)
    print(f"vs 壁(現提出)        : {w}W {l}L {d}D  ({w/max(1,w+l+d):.0%})", flush=True)
    tw = tot = 0
    for p in sorted(glob.glob(os.path.join(HERE, "curmeta", "*.csv"))):
